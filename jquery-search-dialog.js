(function($,document,undefined){
	...
	$.extend(PluginKlass.prototype, {
		...
		fnAjaxSuccessHandler:function(pData, strTextStatus, jqXHR){
			...
			var div=null, label=null, next=null, i=0, iLen=pData.length, self=this;
			for(; i<iLen; i++){
				next=pData[i];
				label=this.fnLabelMapper.call(next,next) || next.toString();
				$button=$selectionButtonBase.clone();
				$button.html(label)
					   .data(strSelectionButtonDataNamespace, next)
					   .on("click" + strPluginEventNamespace, function(e){
					   		self.fnSelectionButtonClick($button);
					   });
		...
		fnSelectionButtonClick:function($button){
			var data=$button.data(strSelectionButtonDataNamespace);
			this.fnSelectionCB.call(this.$input, data);
			this.fnHide();
		},
